...
async function main() {
  ...
  const delegateTx = await contract
    .connect(signer)
    .delegate(JACOB_ADDRESS, { maxPriorityFeePerGas: 3 });
  await delegateTx.wait();
  // Jacob transfering half his voting power to Ian
  // const transferTx = await contract.connect(JACOB_ADDRESS).transfer(IANS_ADDRESS, TEST_MINT_VALUE.div(2));
  // await transferTx.wait();
  // const delegateOtherTx = await contract.connect(IANS_ADDRESS).delegate(IANS_ADDRESS);
